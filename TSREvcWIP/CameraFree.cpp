...
//    playerPos[1] -= przesy;
        playerPos[1] = playerPos[1] - (30.0 / fps) * przesy;
        // Vertical movement historically bypassed check_coords(), allowing
        // Page Down / descend controls to pass through the ground even while
        // Camera Terrain Lock was active. Preserve unrestricted descent in
        // FREE mode and apply the existing terrain clearance in LOCK mode.
        check_coords();
}
...
void CameraFree::MouseMove(QMouseEvent* e) {
    ...
    if (lpm != 0 || rpm != 0) {
        patrzX(Game::mouseSpeed*(float)(starex - position.x())/30.0*(fov/45.0));
        patrzY(Game::mouseSpeed*(float)(starey - position.y())/30.0*(fov/45.0));
    }
    starex = position.x();
    starey = position.y();
}
