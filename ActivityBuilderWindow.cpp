...
ActivityBuilderWindow::ActivityBuilderWindow(ActivityTools *tools, QWidget *parent)
    ...
    QPushButton *operations = new QPushButton(tr("Advanced Path Operations..."), pathControls);
    operations->setEnabled(false);
    operations->setToolTip(tr("Coming in the next operation-editor slice; this button is disabled until its data is saved safely."));
    pathLayout->addWidget(operations);
    ...
    QObject::connect(addWait, &QPushButton::clicked, this,
                     [this, waitUntilTime, waitMinutes, waitSeconds,
    ...
    QObject::connect(viewer, &ActivityTrackViewer::pathPlacementModeChanged, this,
                     [setStart, placeEnd, addReverse, addPassing, addWait](int mode) {
        ...
        addWait->setChecked(mode == ActivityTrackViewer::PlaceWait);
    });
