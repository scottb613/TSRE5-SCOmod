...
void ActivityTrackViewer::choosePathWait(int waitSeconds){
    if(!creatingPath || !draftStartSelected || draftRouteLines.isEmpty()){
        emit pathDraftStatus(tr("<b>Build the main path first</b><br>A wait point must be placed on the magenta path."));
        return;
    }
    if(pathPlacementMode == PlaceWait){
        setPathPlacementMode(PlaceAutomatic);
        emit pathDraftStatus(tr("<b>Add Wait Point cancelled</b>"));
        return;
    }
    pendingWaitSeconds = qBound(1, waitSeconds, 65535);
    setPathPlacementMode(PlaceWait);
    if(pendingWaitSeconds >= 30000 && pendingWaitSeconds < 40000){
        const int clockValue = pendingWaitSeconds - 30000;
        emit pathDraftStatus(tr("<b>Add Wait Until — %1:%2</b><br>Click the magenta path where the train should wait.")
                             .arg(clockValue / 100, 2, 10, QChar('0'))
                             .arg(clockValue % 100, 2, 10, QChar('0')));
    } else {
        emit pathDraftStatus(tr("<b>Add Wait Point — %1 min %2 sec</b><br>Click the magenta path where the train should wait.")
                             .arg(pendingWaitSeconds / 60).arg(pendingWaitSeconds % 60));
    }
    update();
}

void ActivityTrackViewer::setPathPlacementMode(PathPlacementMode mode){
    if(pathPlacementMode == mode)
        return;
...
void ActivityTrackViewer::saveDraftPath(){
    ...
    showTransientMessage(tr("PATH SAVED - %1 nodes, %2 passing path(s)")
                         .arg(controls.size()).arg(draftPassingSidings.size()), false);
    emit pathDraftStatus(tr("<b>Path saved</b><br>%1<br>%2 main/passing nodes written in MSTS/ORTS format.")
                         .arg(selectedPath->pathid.toHtmlEscaped()).arg(controls.size()));
}
...
void ActivityTrackViewer::paintEvent(QPaintEvent *){
    ...
    if(creatingPath && !draftWaitPoints.isEmpty()){
        const qreal pointScale = pixelsPerMeter >= 1.0
        ...
        for(const DraftWaitPoint &wait : draftWaitPoints){
            ...
            painter.drawText(QRectF(center - QPointF(radius, radius),
                             ...
                             Qt::AlignCenter,
                             wait.waitSeconds >= 30000 && wait.waitSeconds < 40000
                                 ? tr("T") : tr("W"));
        }
    }
...
void ActivityTrackViewer::placeDraftPoint(const QPointF &screenPoint, bool moveStart){
    ...
    if(pathPlacementMode == PlaceWait){
        if(!pointOnCurrentFlow(snappedPoint)){
            emit pathDraftStatus(tr("<b>Wait point must be on magenta</b><br>Click directly on the current main path."));
            return;
        }
        int legId = -1;
        qreal best = std::numeric_limits<qreal>::max();
        ...
        if(legId < 0){
            emit pathDraftStatus(tr("<b>Wait point could not be assigned</b><br>Click a visible magenta segment."));
            return;
        }
        DraftWaitPoint wait;
        ...
        if(wait.waitSeconds >= 30000 && wait.waitSeconds < 40000){
            const int clockValue = wait.waitSeconds - 30000;
            emit pathDraftStatus(tr("<b>Wait-until point added</b><br>Train waits until %1:%2. Select it and press Delete to remove it.")
                                 .arg(clockValue / 100, 2, 10, QChar('0'))
                                 .arg(clockValue % 100, 2, 10, QChar('0')));
        } else {
            emit pathDraftStatus(tr("<b>Wait point added</b><br>%1 min %2 sec. Select it and press Delete to remove it.")
                                 .arg(wait.waitSeconds / 60).arg(wait.waitSeconds % 60));
        }
    } else if(pathPlacementMode == PlaceReverse){
        if(!pointOnCurrentFlow(snappedPoint)){
        ...
        if(type == SelectNone){
            for(int i = 0; i < draftWaitPoints.size(); i++){
                if(nearPoint(transform.map(draftWaitPoints[i].position), 14.0)){
                    type = SelectWait;
                    index = i;
                    if(draftWaitPoints[i].waitSeconds >= 30000 &&
                       draftWaitPoints[i].waitSeconds < 40000){
                        const int clockValue = draftWaitPoints[i].waitSeconds - 30000;
                        description = tr("Wait-until point %1 — %2:%3")
                            .arg(i + 1)
                            .arg(clockValue / 100, 2, 10, QChar('0'))
                            .arg(clockValue % 100, 2, 10, QChar('0'));
                    } else {
                        description = tr("Wait point %1 — %2 min %3 sec")
                            .arg(i + 1)
                            .arg(draftWaitPoints[i].waitSeconds / 60)
                            .arg(draftWaitPoints[i].waitSeconds % 60);
                    }
                    break;
                }
