...
void SettingsDialog::createKeyAssignmentsTab(QTabWidget* tabs) {
    ...
    for (int i = 0; i < lines.size(); i++) {
        ...
        int split = line.indexOf(QRegularExpression("\\s{2,}"));
        if (split < 0) {
            if (!firstSection) {
...
void SettingsDialog::setupUi() {
    ...
    addRow(l, "terrainConformTdbBias", "number", "TDB Terrain Height Bias", "metres");
    ...
    QPushButton* saveBtn = new QPushButton("Save settings.json");
    QPushButton* resetDefaultsBtn = new QPushButton("Reset to Defaults...");
    ...
    settingsButtons->addWidget(resetDefaultsBtn);
    ...
    connect(resetDefaultsBtn, &QPushButton::clicked, this, [this]() {
        ...
        if(saveDefaults()){
            QMessageBox::information(
                this, "Settings Reset",
                "Default settings were saved.\n\nRestart TSRE to apply them.");
        } else {
            QMessageBox::warning(
                this, "Settings Reset Failed",
                "TSRE could not replace settings.json.");
        }
    });
...
bool SettingsDialog::save(const QString& filename) {
    const QString settingsPath = filename.isEmpty() ? Game::settingsFilePath() : filename;
    if (!backupSettingsFile(settingsPath))
        return false;
    ...
    writeSetting(out, "terrainConformTdbBias", "0.00", "height added when conforming terrain to TDB");
    ...
    const QStringList lines = legacyLayout.split('\n');
    for(QString line : lines){
        ...
        if(value.compare("true", Qt::CaseInsensitive) == 0)
            settings.insert(key, true);
        else if(value.compare("false", Qt::CaseInsensitive) == 0)
            settings.insert(key, false);
        else if(QRegularExpression("^-?(?:\\d+\\.?\\d*|\\.\\d+)$")
                    .match(value).hasMatch())
            settings.insert(key, value.toDouble());
        else
...
bool SettingsDialog::saveDefaults(const QString& filename) {
    savingBuiltInDefaults = true;
    const bool saved = save(filename);
    savingBuiltInDefaults = false;
    return saved;
}

QString SettingsDialog::currentValue(const QString& key, const QString& fallback) const {
    if(savingBuiltInDefaults)
        return fallback;
...
void SettingsDialog::writeOptionalSetting(QTextStream& out, const QString& key, const QString& fallback, const QString& comment, bool quote) const {
    ...
    QString lowKey = key.toLower();
    bool enabled = !savingBuiltInDefaults && fileActiveMap.value(lowKey, false);
    if (!savingBuiltInDefaults && loadedValueMap.contains(lowKey)
            && val.trimmed() != loadedValueMap.value(lowKey).trimmed())
...
void SettingsDialog::loadSettings() {
    ...
    // 2. Overlay settings from the per-user JSON file (case insensitive).
    QFile file(Game::settingsFilePath());
        ...
        if(error.error == QJsonParseError::NoError && document.isObject()){
            ...
            for(auto it = settings.constBegin(); it != settings.constEnd(); ++it){
                ...
                if(it.value().isBool())
                    ...
                    val = QString::number(it.value().toDouble(), 'g', 15);
...
void SettingsDialog::rememberLoadedValues() {
    loadedValueMap.clear();
    for(auto it = valueWidgetMap.constBegin(); it != valueWidgetMap.constEnd(); ++it)
        loadedValueMap[it.key()] = currentValue(it.key());
}
...
QString SettingsDialog::getGameValue(const QString& key) {
    ...
    if (key == "terrainconformtdbbias") return QString::number(Game::terrainConformTdbBias, 'f', 2);
    if (key == "terrainconformrdbbias") return QString::number(Game::terrainConformRdbBias, 'f', 2);
    ...
    if (key == "skycolor")
        return QColor::fromRgbF(Game::skyColor[0], Game::skyColor[1], Game::skyColor[2]).name();
    if (key == "fogcolor")
        return QColor::fromRgbF(Game::fogColor[0], Game::fogColor[1], Game::fogColor[2]).name();
