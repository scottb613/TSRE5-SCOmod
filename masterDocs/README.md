...
- `tsre-scomod-stable` is the preserved Qt5 rescue line, capped at `v0.8`.
- `TSREvcWIP` is the continuing Qt6/CMake development tree, currently released locally as `v0.9`.
...
Release builds are intended to be published on the GitHub Releases page, not committed directly into the source tree.

Download the current ZIP from Releases when available. Keep this copy separate from any other TSRE installation.
