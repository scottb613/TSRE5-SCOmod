...
$qtVersion = if ($null -ne $localEnvironment) { $localEnvironment.QT_VERSION } else { $null }
$qtArch = if ($null -ne $localEnvironment) { $localEnvironment.QT_ARCH } else { $null }
...
$required = @(
    ...
    @{
        Name = "qmake"
        Candidates = @($(if ($qtRoot -and $qtVersion -and $qtArch) { Join-Path $qtRoot "$qtVersion\$qtArch\bin\qmake.exe" }))
    },
    @{
        Name = "git"
        Candidates = @()
