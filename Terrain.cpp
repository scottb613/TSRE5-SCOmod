...
void Terrain::paintPairedSeasonTexture(Brush* brush, QString textureName, int patchIdx, float x, float z) {
    ...
    // The paired season is painted off-screen and normally has no OpenGL
    // texture until that season is displayed. Keep the CPU image for saving,
    // and only refresh the GPU copy when one actually exists.
    if (TexLib::mtex[texMirrorId[patchIdx]]->glLoaded
            && TexLib::mtex[texMirrorId[patchIdx]]->tex != NULL)
